...
void plot()
{
  ...
  for (int k=0; k<nrun; k++)
  {
    ...
    chain_noavg->Add(Form("../staticCorrOn_scale1_noavgCorr/Reconstructed/%d/clusters_seeds_%d-0.root_resid.root",runs[k],runs[k]));
    TChain* chain_Ben = new TChain("residualtree");
    chain_Ben->Add(Form("../staticCorrOn_scale1_BenMap/Reconstructed/%d/clusters_seeds_%d-0.root_resid.root",runs[k],runs[k]));
    TChain* chain_Xudong = new TChain("residualtree");
    chain_Xudong->Add(Form("../staticCorrOn_scale1_XudongMap/Reconstructed/%d/clusters_seeds_%d-0.root_resid.root",runs[k],runs[k]));
    //chain_Xudong->Add(Form("../staticCorrOn_scale1_XudongMap_2/Reconstructed/%d/clusters_seeds_%d-0.root_resid.root",runs[k],runs[k]));

    TH1* h_quality_noavg = new TH1F("h_quality_noavg","quality;quality;Counts",100, 0,300);
    TH1* h_quality_Ben = new TH1F("h_quality_Ben","quality;quality;Counts",100, 0,300);
    TH1* h_quality_Xudong = new TH1F("h_quality_Xudong","quality;quality;Counts",100, 0,300);
...
void plot1D(TH1* h_noavg, TH1* h_Ben, TH1* h_Xudong, TChain* chain_noavg, TChain* chain_Ben, TChain* chain_Xudong, TString name, TString output)
{
    ...
    TLegend *legend = new TLegend(0.72, 0.75, 0.92, 0.9);
...
void plot2D(TH2* h_noavg, TH2* h_Ben, TH2* h_Xudong, TChain* chain_noavg, TChain* chain_Ben, TChain* chain_Xudong, TString name, TString output)
{
    chain_noavg->Draw((name + " >>" + h_noavg->GetName()), "", "colz");
    chain_Ben->Draw((name + " >>" + h_Ben->GetName()), "", "colz");
    chain_Xudong->Draw((name + " >>" + h_Xudong->GetName()), "", "colz");
    h_noavg->Scale(h_Ben->Integral() / h_noavg->Integral());
    h_Xudong->Scale(h_Ben->Integral() / h_Xudong->Integral());
