void plot1D(TH1* h_noavg, TChain* chain_noavg, TString name, TString output);
...
void plot()
{
  ...
  for (int k=0; k<nrun; k++)
  {
    ...
    plot1D(h_quality_noavg, chain_noavg, Form("quality"), Form("figure/%d_quality.pdf",runs[k]));
...
void plot1D(TH1* h_noavg, TChain* chain_noavg, TString name, TString output)
{
    ...
    h_noavg->Draw("hist");
    TLegend *legend = new TLegend(0.35, 0.75, 0.72, 0.9);
    legend->SetHeader("Run 53877, no average correction");
    legend->Draw("same");
    can->Update();
    can->SaveAs(output);
}
