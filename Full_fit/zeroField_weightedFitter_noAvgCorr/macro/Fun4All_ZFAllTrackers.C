...
void Fun4All_ZFAllTrackers(
    ...
    const std::string clusterfilename = "DST_TRKR_CLUSTER_run2pp_ana517_2024p024_v001-00052077-00000.root",
    ...
    const bool convertSeeds = false)
{
  ...
  G4TPC::tpc_drift_velocity_reco = 0.0073821095; // cm/ns
  TpcClusterZCrossingCorrection::_vdrift = G4TPC::tpc_drift_velocity_reco;
  G4TPC::tpc_tzero_reco = -15.483324;  // ns
  ...
  //G4TPC::average_correction_filename = "/sphenix/u/xyu3/workarea/TPCdistortion/Si_TPOT_fit/zeroField_weightedFitter/jobB/Rootfiles/Distortions_full_mm_52077.root";
  ...
  */
  ...
  std::string weighted_fitter_outfile_string;
  /*
  ...
  else
  {
    ...
    const TString weighted_fitter_outfile = theOutfile + "_FullTrackWeightedFitter.root";
    weighted_fitter_outfile_string = weighted_fitter_outfile.Data();
    std::cout<<"weighted_fitter_outfile_string = "<<weighted_fitter_outfile_string<<std::endl;

    auto silicon_tpot_weighted_fitter = new WeightedFitter("FullTrackWeightedFitter");
    silicon_tpot_weighted_fitter->Verbosity(0);
    //silicon_tpot_weighted_fitter->use_vertex();
    silicon_tpot_weighted_fitter->set_which_tracks(WeightedFitter::k_svtx_tracks);
    //for (int i=0; i<=2; i++) {silicon_tpot_weighted_fitter->include_layer_in_fit(i);} //mvtx
    //for (int i=3; i<=6; i++) {silicon_tpot_weighted_fitter->include_layer_in_fit(i);} //intt
    //for (int i=55; i<=56; i++) {silicon_tpot_weighted_fitter->include_layer_in_fit(i);} //tpot
    silicon_tpot_weighted_fitter->set_min_num_mvtx(3);
    silicon_tpot_weighted_fitter->set_min_num_intt(2);
    silicon_tpot_weighted_fitter->set_min_num_tpc(30);
    //silicon_tpot_weighted_fitter->set_min_num_tpot(1);
    silicon_tpot_weighted_fitter->set_track_map_node_name("FullTrackWeightedFitterSvtxTrackMap");
    silicon_tpot_weighted_fitter->set_alignment_map_node_name("FullTrackWeightedFitterAlignmentStateMap");
    silicon_tpot_weighted_fitter->set_weighted_track_map_node_name("FullTrackWeightedFitterTrackMap");

    silicon_tpot_weighted_fitter->set_ntuple_file_name(weighted_fitter_outfile_string);
    se->registerSubsystem(silicon_tpot_weighted_fitter);
  }
  ...
  finder->setTrackMapName("FullTrackWeightedFitterSvtxTrackMap");
  ...
  if (G4TRACKING::SC_CALIBMODE)
  {
    ...
    */
    ...
    residuals->setTrackMapName("FullTrackWeightedFitterSvtxTrackMap");
    ...
    residuals->setMaxTrackResidualDrphi_TPOT(0.5);
    residuals->setMaxTrackResidualDz_TPOT(2);
  ...
  if(G4TRACKING::SC_CALIBMODE && !G4TRACKING::convert_seeds_to_svtxtracks)
  {
    ...
    resid->trackmapName("FullTrackWeightedFitterSvtxTrackMap");
  ...
  */
  ...
  ifstream file_weighted_fitter(weighted_fitter_outfile_string.c_str(), ios::binary | ios::ate);
  if (file_weighted_fitter.good() && (file_weighted_fitter.tellg() > 100))
  {
    string outputDirMove = outdir + "/Reconstructed/" + to_string(runnumber) + "/";
    string makeDirectoryMove = "mkdir -p " + outputDirMove;
    system(makeDirectoryMove.c_str());
    string moveOutput = "mv " + weighted_fitter_outfile_string + " " + outputDirMove;
    std::cout << "moveOutput: " << moveOutput << std::endl;
    system(moveOutput.c_str());
  }
