...
void plot1D_QoverPt(TH1* h[], TChain* chain, TString name, TCut cut, TString output, double fit_xmin, double fit_xmax, bool logy=0, bool doexpfit=0);
...
const int nQoverPtbin=16;
double QoverPt_min[nQoverPtbin]={-2.11,-1.90,-1.66,-1.38,-1.00,-0.33,0.00,0.20,0.33,0.81,1.00,1.11,1.34,1.55,1.75,1.94};
double QoverPt_max[nQoverPtbin]={-1.90,-1.66,-1.38,-1.00,-0.33,0.00,0.20,0.33,0.81,1.00,1.11,1.34,1.55,1.75,1.94,2.13};
...
void plot()
{
  ...
  for (int k=0; k<nrun; k++)
  {
    ...
    plot1D_QoverPt(h1_dx_QoverPt, chain, Form("m_tpcseedx-m_silseedx"), cut, Form("figure/%d_dx_allQoverPt",runs[k]), -5, 5, 0);
    ...
    plot1D_QoverPt(h1_dy_QoverPt, chain, Form("m_tpcseedy-m_silseedy"), cut, Form("figure/%d_dy_allQoverPt",runs[k]), -5, 5, 0);

    TH1* h1_dz_QoverPt[nQoverPtbin];
    for (int i=0; i<nQoverPtbin; i++)
    {
      h1_dz_QoverPt[i] = new TH1F(Form("h1_dz_QoverPt_%d",i),"TPCz - Siz;z_{TPC}-z_{Si};Counts",100,-4,4);
    }
    plot1D_QoverPt(h1_dz_QoverPt, chain, Form("m_tpcseedz-m_silseedz"), cut, Form("figure/%d_dz_allQoverPt",runs[k]), -4, 4, 0, 1);
    */

    TH1* h1_dz_etapos_QoverPt[nQoverPtbin];
    for (int i=0; i<nQoverPtbin; i++)
    {
      h1_dz_etapos_QoverPt[i] = new TH1F(Form("h1_dz_etapos_QoverPt_%d",i),"TPCz - Siz #eta>0;z_{TPC}-z_{Si};Counts",100,-4,4);
    }
    plot1D_QoverPt(h1_dz_etapos_QoverPt, chain, Form("m_tpcseedz-m_silseedz"), cut+"tpcseedeta>0", Form("figure/%d_dz_allQoverPt_etapos",runs[k]), -4, 4, 0);

    /*
    TH1* h1_dz_etaneg_QoverPt[nQoverPtbin];
    for (int i=0; i<nQoverPtbin; i++)
    {
      h1_dz_etaneg_QoverPt[i] = new TH1F(Form("h1_dz_etaneg_QoverPt_%d",i),"TPCz - Siz #eta<0;z_{TPC}-z_{Si};Counts",100,-4,4);
    }
    plot1D_QoverPt(h1_dz_etaneg_QoverPt, chain, Form("m_tpcseedz-m_silseedz"), cut+"tpcseedeta<0", Form("figure/%d_dz_allQoverPt_etaneg",runs[k]), -4, 4, 0);

    TH1* h1_deta_QoverPt[nQoverPtbin];
    for (int i=0; i<nQoverPtbin; i++)
    {
      h1_deta_QoverPt[i] = new TH1F(Form("h1_deta_QoverPt_%d",i),"TPC#eta - Si#eta;#eta_{TPC}-#eta_{Si};Counts",100,-0.1,0.1);
    }
    plot1D_QoverPt(h1_deta_QoverPt, chain, Form("m_tpcseedeta-m_silseedeta"), cut, Form("figure/%d_deta_allQoverPt",runs[k]), -0.1, 0.1, 0, 1);
    ...
    plot1D_QoverPt(h1_dphi_QoverPt, chain, Form("m_tpcseedphi-m_silseedphi"), cut, Form("figure/%d_dphi_allQoverPt",runs[k]), -0.2, 0.2, 0);
    */
  }
}
...
void plot1D_QoverPt(TH1* h[], TChain* chain, TString name, TCut cut, TString output, double fit_xmin, double fit_xmax, bool logy=0, bool doexpfit=0)
{
    ...
    Double_t arr_3sigma[nQoverPtbin];
    Double_t arr_3sigmawidnow[nQoverPtbin], arr_3sigmawindow_error[nQoverPtbin];
    ...
    for (int i=0; i<nQoverPtbin; i++)
    {
      ...
      arr_sigma[i] = sigma;
      arr_3sigma[i] = 3*sigma;
      arr_sigma_error[i] = sigma_error;
      if (mean>0)
      {
        arr_3sigmawidnow[i] = mean + 3*sigma;
      }
      else if (mean<0)
      {
        arr_3sigmawidnow[i] = fabs(mean - 3*sigma);
      }
      arr_3sigmawindow_error[i] = sqrt(pow(mean_error,2)+pow(3*sigma_error,2));
    }

    if (doexpfit)
    {
      TGraphErrors* gr_3sigmawindow = new TGraphErrors(nQoverPtbin, QoverPt_mid, arr_3sigmawidnow, QoverPt_interval, arr_3sigmawindow_error);
      gr_3sigmawindow->GetXaxis()->SetTitle("Q_{TPC}/pT_{TPC}");
      gr_3sigmawindow->GetYaxis()->SetTitle("Max Gaussian #mu #pm 3#sigma");
      //gr_3sigmawindow->SetMaximum(1);

      TF1 *fexp_neg = new TF1("fexp_neg", "[0]+[1]*exp(-[2]*x)", QoverPt_mid[0], 0);
      fexp_neg->SetParameter(0, arr_3sigmawidnow[nQoverPtbin/2-1]);
      fexp_neg->SetParameter(1, 0.005);
      fexp_neg->SetParameter(2, 1);
      gr_3sigmawindow->Fit(fexp_neg, "Q", "", QoverPt_mid[0], 0);
      ...
      fexp_pos->SetParameter(0, arr_3sigmawidnow[nQoverPtbin/2-1]);
      fexp_pos->SetParameter(1, 0.005);
      fexp_pos->SetParameter(2, 1);
      gr_3sigmawindow->Fit(fexp_pos, "Q", "", 0, QoverPt_mid[nQoverPtbin-1]);
    ...
    TGraphErrors* gr_mean_3sigma = new TGraphErrors(nQoverPtbin, QoverPt_mid, arr_mean, QoverPt_interval, arr_3sigma);
    ...
    gr_mean_3sigma->GetXaxis()->SetTitle("Q_{TPC}/pT_{TPC}");
    gr_mean_3sigma->GetYaxis()->SetTitle("Gaussian #mu #pm 3#sigma");
