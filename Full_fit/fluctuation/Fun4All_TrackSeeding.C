...
void Fun4All_TrackSeeding(
    ...
    const std::string laserfilename = "DST_LASER_CLUSTER_run2pp_ana464_nocdbtag_v001-00053877-00000.root",
    const std::string laserdir = "/sphenix/tg/tg01/jets/bkimelman/BenProduction/Apr7_2025/",
    ...
    const bool convertSeeds = false)
{
  ...
  std::string inputlaserFile = laserdir + laserfilename;
  ...
  TString outfile = outDir + outfilename + "_" + runnumber + "-" + segment;
  ...
  auto hitsinlaser = new Fun4AllDstInputManager("LaserInputManager");
  hitsinlaser->fileopen(inputlaserFile);
  se->registerInputManager(hitsinlaser);

  TString cmoutfile = theOutfile + "_cm.root";
  std::string cmstring(cmoutfile.Data());
  ...
  //centralMembraneMatcher->setOutputfile(Form("%s/CMDistortionCorrections-%08d-%05d.root",outdir.c_str(),runnumber,segment));
  centralMembraneMatcher->setOutputfile(cmstring);
  ...
  */
  ...
  ifstream file_cm(cmstring.c_str(), ios::binary | ios::ate);
  if (file_cm.good() && (file_cm.tellg() > 100))
  {
    string outputDstDirMove = outdir + "/Reconstructed/" + to_string(runnumber) + "/";
    string makeDirectoryMove = "mkdir -p " + outputDstDirMove;
    system(makeDirectoryMove.c_str());
    string moveOutput = "mv " + cmstring + " " + outputDstDirMove;
    std::cout << "moveOutput: " << moveOutput << std::endl;
    system(moveOutput.c_str());
  }
