...
for (int i = 0; i < nrun; i++)
{
  ...
  draw1Dmap_2D(cdbfilename_lamination[i].c_str(), Form("%d_lamination_cdb",runs[i]), selectR, lcdb_lamination_R_pos[i], lcdb_lamination_R_neg[i], lcdb_lamination_P_pos[i], lcdb_lamination_P_neg[i], lcdb_lamination_Z_pos[i], lcdb_lamination_Z_neg[i], kBlack);

  draw1Dmap(Form("./Rootfiles/Distortions_full_mm_%d.root",runs[i]), Form("%d_TPOT",runs[i]), selectR, h_R_pos[i], h_R_neg[i], h_P_pos[i], h_P_neg[i], h_Z_pos[i], h_Z_neg[i], kRed);

  draw1Dmap(Form("/sphenix/tg/tg01/jets/hangal/distortions/run26_pp/distortion_spacecharge_run%d_mean.distortion_map.hist.root",runs[i]), Form("%d_DC",runs[i]), selectR, h_DC_R_pos[i], h_DC_R_neg[i], h_DC_P_pos[i], h_DC_P_neg[i], h_DC_Z_pos[i], h_DC_Z_neg[i], kBlue);
}
...
for (int i=0; i<nrun; i++)
{
  ...
  h_DC_R_pos[i]->Write();
  h_DC_R_neg[i]->Write();
  lcdb_lamination_R_pos[i]->Write();
  lcdb_lamination_R_neg[i]->Write();
}
ofile->Write();
...
for (int i=0; i<nrun; i++)
{
  ...
  h_DC_P_pos[i]->Write();
  h_DC_P_neg[i]->Write();
  lcdb_lamination_P_pos[i]->Write();
  lcdb_lamination_P_neg[i]->Write();
}
ofile2->Write();
...
for (int i=0; i<nrun; i++)
{
  ...
  h_DC_Z_pos[i]->Write();
  h_DC_Z_neg[i]->Write();
  lcdb_lamination_Z_pos[i]->Write();
  lcdb_lamination_Z_neg[i]->Write();
}
ofile3->Write();
...
lcdb_lamination_P_pos[i]->Draw("hist,e,same");
l_nco_i_P[i]->Draw();
l_nco_o_P[i]->Draw();
l_nci_i_P[i]->Draw();
l_nci_o_P[i]->Draw();
...
lcdb_lamination_R_pos[i]->Draw("hist,e,same");
l_nco_i_R[i]->Draw();
l_nco_o_R[i]->Draw();
l_nci_i_R[i]->Draw();
l_nci_o_R[i]->Draw();
...
lcdb_lamination_Z_pos[i]->Draw("hist,e,same");
l_nco_i_Z[i]->Draw();
l_nco_o_Z[i]->Draw();
l_nci_i_Z[i]->Draw();
l_nci_o_Z[i]->Draw();
...
lcdb_lamination_P_neg[i]->Draw("hist,e,same");
l_sco_i_P[i]->Draw();
l_sco_o_P[i]->Draw();
l_sci_i_P[i]->Draw();
l_sci_o_P[i]->Draw();
...
lcdb_lamination_R_neg[i]->Draw("hist,e,same");
l_sco_i_R[i]->Draw();
l_sco_o_R[i]->Draw();
l_sci_i_R[i]->Draw();
l_sci_o_R[i]->Draw();
...
lcdb_lamination_Z_neg[i]->Draw("hist,e,same");
l_sco_i_Z[i]->Draw();
l_sco_o_Z[i]->Draw();
l_sci_i_Z[i]->Draw();
l_sci_o_Z[i]->Draw();
...
legend->AddEntry(lcdb_lamination_P_pos[i], Form("Lamination"), "f");
legend->Draw();
