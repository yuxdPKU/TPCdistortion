...
//_______________________________________________
void DistortionCorrectionMatrixInversion(int run=29, TString type = "MI_sim_reco_acts")
{
  ...
  const TString inputFile = Form( "../root/Reconstructed/%d/%s_%d-*.root_PhTpcResiduals.root", run, type.Data(), run );
  ...
  // output file
  const TString outputFile = Form( "Rootfiles/Distortions_full_mm_%d_%s.root", run, type.Data() );
  const TString outputFile_phi = Form( "Rootfiles/Distortions_full_mm_%d_%s_phi.root", run, type.Data() );
  const TString outputFile_z = Form( "Rootfiles/Distortions_full_mm_%d_%s_z.root", run, type.Data() );
  const TString outputFile1D = Form( "Rootfiles/Distortions_1D_mm_%d_%s.root", run, type.Data() );
  const TString outputFile2D = Form( "Rootfiles/Distortions_2D_mm_%d_%s_rz.root", run, type.Data() );
  ...
  */
  ...
  spaceChargeMatrixInversion.set_min_cluster_count(20);
  ...
  spaceChargeMatrixInversion_phi.set_min_cluster_count(20);
  ...
  // calculate the distortions in TPOT acceptance
  spaceChargeMatrixInversion_phi.calculate_distortion_corrections(TpcSpaceChargeMatrixInversion::InversionMode::ReducedInversion_phi);
  ...
  spaceChargeMatrixInversion_r.set_min_cluster_count(20);
  ...
  // calculate the distortions in TPOT acceptance
  spaceChargeMatrixInversion_r.calculate_distortion_corrections(TpcSpaceChargeMatrixInversion::InversionMode::ReducedInversion_z);
  ...
  // write to output
  spaceChargeMatrixInversion_r.save_distortion_corrections( outputFile_z.Data() );
