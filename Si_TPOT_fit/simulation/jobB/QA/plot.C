void fit_gauss_Rslice(TH2* h, TString name, bool verbose=0, TH1* h_cdb=nullptr, TH1* h_sitpot=nullptr);
...
void plot()
{
  ...
  for (int k=0; k<nrun; k++)
  {
    ...
    for (int i=0; i<1000; i++) {intree->Add(Form("../../root/Reconstructed/%d/clusters_seeds_%d-%d-0.root_qa.root",runs[k],runs[k],i));}
    ...
    TH2* h2_drphi_r = new TH2F("h2_drphi_r","Rd#phi vs. R @ |Z|<20;R (cm);Rd#phi (cm)",16,20,78,50,-1,1);
    const int nbin_z = 6;
    ...
    {
      arr_h2_drphi_r_posz[i] = new TH2F(Form("h2_drphi_r_atposZ%d",(int)arr_z[i]),Form("Rd#phi vs. R @ Z=%d#pm2cm;R (cm);Rd#phi (cm)",(int)arr_z[i]),16,20,78,50,-1,1);
      arr_h2_drphi_r_negz[i] = new TH2F(Form("h2_drphi_r_atnegZ%d",(int)arr_z[i]),Form("Rd#phi vs. R @ Z=%d#pm2cm;R (cm);Rd#phi (cm)",-1*(int)arr_z[i]),16,20,78,50,-1,1);
    ...
    */
    ...
    for (int i=0; i<nbin_z; i++)
    {
      // get cdb hist and si-tpot matrix inversion hist
      TFile* inmyfile = new TFile(Form("../Rootfiles/hist_rdphi_1Dr_Z%d.root",(int)arr_z[i]),"");
      inmyfile->cd();
      TH1* h1_rdphi_cdb_posz = (TH1*) inmyfile->Get(Form("hIntDistortionP_posz_%d_cdb",runs[k]));
      TH1* h1_rdphi_cdb_negz = (TH1*) inmyfile->Get(Form("hIntDistortionP_negz_%d_cdb",runs[k]));
      TH1* h1_rdphi_sitpot_posz = (TH1*) inmyfile->Get(Form("hIntDistortionP_posz_%d",runs[k]));
      TH1* h1_rdphi_sitpot_negz = (TH1*) inmyfile->Get(Form("hIntDistortionP_negz_%d",runs[k]));

      fit_gauss_Rslice(arr_h2_drphi_r_posz[i], Form("figure/%d_drphi_r_posz%d.pdf",runs[k],(int)arr_z[i]),0,h1_rdphi_cdb_posz,h1_rdphi_sitpot_posz);
      fit_gauss_Rslice(arr_h2_drphi_r_negz[i], Form("figure/%d_drphi_r_negz%d.pdf",runs[k],(int)arr_z[i]),0,h1_rdphi_cdb_negz,h1_rdphi_sitpot_negz);
...
void fit_gauss_Rslice(TH2* h, TString name, bool verbose=0, TH1* h_cdb=nullptr, TH1* h_sitpot=nullptr)
{
  ...
  {
    cout<<"Plot CDB"<<endl;
    h_cdb->SetLineColor(1); h_cdb->SetLineWidth(1); h_cdb->SetFillColor(0);
    h_cdb->Draw("hist,same");
  }

  if (h_sitpot)
  {
    cout<<"Plot CDB"<<endl;
    h_sitpot->SetLineColor(2); h_sitpot->SetLineWidth(1); h_sitpot->SetFillColor(0);
    h_sitpot->Draw("hist,same");
  }
