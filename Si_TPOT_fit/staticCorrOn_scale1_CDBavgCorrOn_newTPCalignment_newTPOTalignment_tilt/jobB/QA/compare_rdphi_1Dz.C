...
TH1 *h1_RP_rphi_posz[nrun], *h1_RP_matrix_posz[nrun];
TH1 *h1_RP_rphi_negz[nrun], *h1_RP_matrix_negz[nrun];
...
TFile* infile_matrix = new TFile(Form("../Rootfiles/hist_rdphi_1Dz_R%d.root",(int)selectR));
infile_matrix->cd();
for (int i=0; i<nrun; i++)
{
...
can->cd(4);
legend_negz->Draw();
can->SaveAs(Form("figure/rdphi_vsZ_compare_R%d.pdf",(int)selectR));
