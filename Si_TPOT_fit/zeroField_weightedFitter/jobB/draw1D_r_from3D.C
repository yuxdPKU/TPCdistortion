...
for (int i = 0; i < nrun; i++)
{
  ...
  //cdbfilename[i] = "/sphenix/tg/tg01/jets/bkimelman/BenProduction/Dec5_2025_LaminationFits_fieldOffBeamOn_52077/Laminations_run2pp_ana517_2024p024_v001-00052077.root";
  cdbfilename[i] = "/sphenix/tg/tg01/jets/bkimelman/BenProduction/Dec17_2025_LaminationFits_fieldOffBeamOn_52077/Laminations_run2pp_ana517_2024p024_v001-00052077.root";
...
for (int i=0; i<nrun; i++) hcdb_P_pos[i]->Draw("hist,same");
can->cd(2);
gPad->SetLogy(0);
...
for (int i=0; i<nrun; i++) hcdb_Z_pos[i]->Draw("hist,same");
can->cd(4);
gPad->SetLogy(1);
for (int i=0; i<nrun; i++) h_N_pos[i]->Draw("hist,e,same");
//for (int i=0; i<nrun; i++) hcdb_N_pos[i]->Draw("hist,same");
can->cd(5);
gPad->SetLogy(0);
for (int i=0; i<nrun; i++) h_P_neg[i]->Draw("hist,e,same");
for (int i=0; i<nrun; i++) hcdb_P_neg[i]->Draw("hist,same");
can->cd(6);
gPad->SetLogy(0);
...
for (int i=0; i<nrun; i++) hcdb_Z_neg[i]->Draw("hist,same");
can->cd(8);
gPad->SetLogy(1);
for (int i=0; i<nrun; i++) h_N_neg[i]->Draw("hist,e,same");
//for (int i=0; i<nrun; i++) hcdb_N_neg[i]->Draw("hist,same");
...
for (int i=0; i<nrun; i++) legend->AddEntry(hcdb_P_pos[i], Form("Run %d, Lamination",runs[i]), "l");
legend->Draw();
