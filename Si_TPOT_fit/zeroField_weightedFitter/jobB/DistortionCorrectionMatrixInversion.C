...
//53285 - 70khz
  ...
  const TString inputFile = Form( "../root/Reconstructed/%d/clusters_seeds_%d-*.root_PhTpcResiduals.root", run, run );
  ...
  */
  ...
  spaceChargeMatrixInversion.set_min_cluster_count(100);

  std::cout<<__LINE__<<std::endl;
  // load input files
  for( const auto& file:filenames )
  { std::cout<<file<<std::endl; spaceChargeMatrixInversion.add_from_file( file , "TpcSpaceChargeMatrixContainer"); }
  ...
  spaceChargeMatrixInversion_phi.set_min_cluster_count(100);
  ...
  spaceChargeMatrixInversion_r.set_min_cluster_count(100);
