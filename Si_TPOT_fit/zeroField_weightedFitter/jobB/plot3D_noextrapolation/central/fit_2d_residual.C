    ...
    for (int ir=0; ir<nRslides; ir++)
    {
      ...
      for (int i=0; i<nevent; i++)
      {
        if (i % (Long64_t)(nevent / 10) == 0) std::cout << "progress: " << i / (Long64_t)(nevent / 10) << "0%" << std::endl;
          ...
          if (std::abs(tanAlpha) > 20e-3 || std::abs(drphi) > 2) continue;
      ...
      for (int j = 0; j < nZslides_posz; j++)
      {
        ...
        if (gr_drphi_tanAlpha_posz[j]->GetN() < 100) continue;
        if (gr_dz_tanBeta_posz[j]->GetN() < 100) continue;

	// Perform the fit (equal weights for all points)
        gr_drphi_tanAlpha_posz[j]->Fit(Form("fit_drphi_tanAlpha_posz_R%d_Z%d",ir,j), "Q", "", -5e-3, 5e-3); // "Q" for quiet mode
	  ...
	  can_drphi_tanAlpha_posz->SetRightMargin(0.1);
	  ...
	  can_dz_tanBeta_posz->SetRightMargin(0.1);
      ...
      for (int j = 0; j < nZslides_negz; j++)
      {
        ...
        if (gr_drphi_tanAlpha_negz[j]->GetN() < 100) continue;
        if (gr_dz_tanBeta_negz[j]->GetN() < 100) continue;

        // Perform the fit (equal weights for all points)
        gr_drphi_tanAlpha_negz[j]->Fit(Form("fit_drphi_tanAlpha_negz_R%d_Z%d",ir,j), "Q", "", -5e-3, 5e-3); // "Q" for quiet mode
	  ...
	  can_drphi_tanAlpha_negz->SetRightMargin(0.1);
	  ...
	  can_dz_tanBeta_negz->SetRightMargin(0.1);
      ...
      if (h_dr_rphi_posz->GetMinimum() < h_dr_zr_posz->GetMinimum()) h_dr_rphi_posz->SetMinimum(h_dr_rphi_posz->GetMinimum());
      ...
      if (h_dr_rphi_negz->GetMinimum() < h_dr_zr_negz->GetMinimum()) h_dr_rphi_negz->SetMinimum(h_dr_rphi_negz->GetMinimum());
