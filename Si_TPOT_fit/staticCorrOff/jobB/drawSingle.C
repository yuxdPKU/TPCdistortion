void saveTH1asPDF(TString inputFileName, TString outputDir) {
    ...
    while ((key = (TKey*)next())) {
        ...
        if (obj && obj->InheritsFrom(TH1::Class())) {
            ...
            TString outputFileName = Form("%s/%s.pdf", outputDir.Data(), histName.Data());
...
void drawSingle() {
    ...
    for(int i=0; i<7; i++)
    {
      TString inputFileName = Form("Rootfiles/Distortions_1D_mm_%d.root", runs[i]);
      TString outputDir = Form("figure_%d",runs[i]);
      gSystem->mkdir(outputDir, kTRUE);
      saveTH1asPDF(inputFileName, outputDir);
    }
}
