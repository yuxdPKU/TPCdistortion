...
void makeplot_error()
{
  ...
  for (int k=0; k<nrun; k++)
  {
    ...
    TH1* h1_cluster_rphierr = new TH1F("h1_cluster_rphierr","; RPhierr (cm); Entries",100,0,0.2);
    TH1* h1_state_rphierr = new TH1F("h1_state_rphierr","; RPhierr (cm); Entries",100,0,0.2);
    ...
    if (h1_cluster_zerr->GetMaximum() > h1_state_zerr->GetMaximum()) { h1_state_zerr->SetMaximum(1.1 * h1_cluster_zerr->GetMaximum()); }
    h1_state_zerr->Draw("hist");
    h1_cluster_zerr->Draw("hist,same");
    TLegend *legend_zerr = new TLegend(0.60, 0.70, 0.80, 0.90);
    ...
    if (h1_cluster_rphierr->GetMaximum() > h1_state_rphierr->GetMaximum()) { h1_state_rphierr->SetMaximum(1.1 * h1_cluster_rphierr->GetMaximum()); }
    h1_state_rphierr->Draw("hist");
    h1_cluster_rphierr->Draw("hist,same");
    TLegend *legend_rphierr = new TLegend(0.60, 0.70, 0.80, 0.90);
