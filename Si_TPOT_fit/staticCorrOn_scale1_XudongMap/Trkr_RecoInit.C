...
void TrackingInit()
{
  ...
  if (G4TPC::ENABLE_MODULE_EDGE_CORRECTIONS || G4TPC::ENABLE_STATIC_CORRECTIONS || G4TPC::ENABLE_AVERAGE_CORRECTIONS)
  {
    ...
    if (G4TPC::ENABLE_STATIC_CORRECTIONS)
    {
      ...
      float scale=1.0;
      std::cout<<"Hey!!! set static correction scale factor to "<<scale<<std::endl;
      tpcLoadDistortionCorrection->set_scale_factor(TpcLoadDistortionCorrection::DistortionType_Static, scale);
    }
