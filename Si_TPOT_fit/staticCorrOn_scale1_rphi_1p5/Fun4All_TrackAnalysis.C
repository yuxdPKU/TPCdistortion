  ...
  if(G4TPC::ENABLE_AVERAGE_CORRECTIONS)
    {
      ...
      std::array<double,3> arrlo = {-0.12,0,0};
      std::array<double,3> arrhi = {0.08,0,0};
      silicon_match->window_dphi.set_QoverpT_range(arrlo, arrhi);
      arrhi = {2.,0,0};
      silicon_match->window_dx.set_QoverpT_maxabs(arrhi);
      arrhi = {1.5,0,0};
      silicon_match->window_dy.set_QoverpT_maxabs(arrhi);
      std::array<double,3> arrhi_pos = {0.019,0.0050,1.18};
      std::array<double,3> arrhi_neg = {0.027,0.0009,1.83};
      silicon_match->window_deta.set_posQoverpT_maxabs(arrhi_pos);
      silicon_match->window_deta.set_negQoverpT_maxabs(arrhi_neg);
    }
