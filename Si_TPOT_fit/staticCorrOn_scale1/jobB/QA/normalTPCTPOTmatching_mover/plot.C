...
void plot()
{
  ...
  for (int k=0; k<nrun; k++)
  {
    ...
    float drphi_mover, dz_mover, clusZ_mover, clusR_mover, clusPhi_mover;
    float tanAlpha, tanBeta;
    float tanAlpha_mover, tanBeta_mover;
    float trackPt;
    int layer, charge;
    ...
    intree->SetBranchAddress("dz_mover",&dz_mover);
    intree->SetBranchAddress("drphi",&drphi);
    intree->SetBranchAddress("drphi_mover",&drphi_mover);
    intree->SetBranchAddress("clusZErr",&clusZErr);
    intree->SetBranchAddress("stateZErr",&stateZErr);
    intree->SetBranchAddress("clusZ",&clusZ);
    intree->SetBranchAddress("clusZ_mover",&clusZ_mover);
    intree->SetBranchAddress("clusR",&clusR);
    intree->SetBranchAddress("clusR_mover",&clusR_mover);
    intree->SetBranchAddress("clusPhi",&clusPhi);
    intree->SetBranchAddress("clusPhi_mover",&clusPhi_mover);
    ...
    intree->SetBranchAddress("tanAlpha_mover",&tanAlpha_mover);
    intree->SetBranchAddress("trackPt",&trackPt);
    intree->SetBranchAddress("tanBeta",&tanBeta);
    intree->SetBranchAddress("tanBeta_mover",&tanBeta_mover);
    ...
    for (int i=0; i<nevent; i++)
    {
      ...
      if (std::abs(tanAlpha_mover) > 0.6 || std::abs(drphi_mover) > 2) continue;
      if (std::abs(tanBeta_mover) > 1.5 || std::abs(dz_mover) > 5) continue;
      if (track_nmvtxstate<3) continue;
      if (track_ninttstate<2) continue;
      if (track_ntpc<35) continue;
      if (track_ntpotstate<2) continue;
  
      if (std::fabs(clusZ_mover)>=20) continue;
      if (layer_now>layer_last)
      {
	// still the same track
        vec_clusZ.push_back(clusZ_mover);
        vec_clusR.push_back(clusR_mover);
        vec_drphi.push_back(drphi_mover);
      }
      else
      {
        // another track
