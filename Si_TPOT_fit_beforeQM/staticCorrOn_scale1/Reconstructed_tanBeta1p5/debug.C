void debug()
{
  ...
  TFile* infile = new TFile("allqa_53630.root");
  TTree* intree = (TTree*) infile->Get("h_QAG4SimulationDistortions_residTree");
  float drphi, dz, clusZErr, stateZErr, clusZ, clusR, clusRPhiErr, stateRPhiErr;
  ...
  can->Update();
  can->SaveAs("figure/53630_residual_clusz_r6468.pdf");
}
