...
void plot1D(TH2* h2, TH1* h1, double r0 = 40)
{
  int ybin = h2->GetYaxis()->FindBin(r0);
  for (int i = 1; i <= h2->GetNbinsX(); i++)
  {
      float bincontent = h2->GetBinContent(i, ybin);
      float binerror = h2->GetBinError(i, ybin);
...
for (int i = 0; i < nrun; i++)
{
  ...
  hcdb_N_pos[i] = new TH1F(Form("hentries_posz_%d_cdb",runs[i]),Form("N;#phi (rad);N"),hcdb_N_pr_pos[i]->GetXaxis()->GetNbins(),hcdb_N_pr_pos[i]->GetXaxis()->GetXmin(),hcdb_N_pr_pos[i]->GetXaxis()->GetXmax());
  hcdb_R_pos[i] = new TH1F(Form("hIntDistortionR_posz_%d_cdb",runs[i]),Form("dR;#phi (rad);dR (cm)"),hcdb_R_pr_pos[i]->GetXaxis()->GetNbins(),hcdb_R_pr_pos[i]->GetXaxis()->GetXmin(),hcdb_R_pr_pos[i]->GetXaxis()->GetXmax());
  hcdb_P_pos[i] = new TH1F(Form("hIntDistortionP_posz_%d_cdb",runs[i]),Form("Rdphi;#phi (rad);Rdphi (rad)"),hcdb_P_pr_pos[i]->GetXaxis()->GetNbins(),hcdb_P_pr_pos[i]->GetXaxis()->GetXmin(),hcdb_P_pr_pos[i]->GetXaxis()->GetXmax());
  hcdb_Z_pos[i] = new TH1F(Form("hIntDistortionZ_posz_%d_cdb",runs[i]),Form("dz;#phi (rad);dz (cm)"),hcdb_Z_pr_pos[i]->GetXaxis()->GetNbins(),hcdb_Z_pr_pos[i]->GetXaxis()->GetXmin(),hcdb_Z_pr_pos[i]->GetXaxis()->GetXmax());
  hcdb_N_neg[i] = new TH1F(Form("hentries_negz_%d_cdb",runs[i]),Form("N;#phi (rad);N"),hcdb_N_pr_neg[i]->GetXaxis()->GetNbins(),hcdb_N_pr_neg[i]->GetXaxis()->GetXmin(),hcdb_N_pr_neg[i]->GetXaxis()->GetXmax());
  hcdb_R_neg[i] = new TH1F(Form("hIntDistortionR_negz_%d_cdb",runs[i]),Form("dR;#phi (rad);dR (cm)"),hcdb_R_pr_neg[i]->GetXaxis()->GetNbins(),hcdb_R_pr_neg[i]->GetXaxis()->GetXmin(),hcdb_R_pr_neg[i]->GetXaxis()->GetXmax());
  hcdb_P_neg[i] = new TH1F(Form("hIntDistortionP_negz_%d_cdb",runs[i]),Form("Rdphi;#phi (rad);Rdphi (rad)"),hcdb_P_pr_neg[i]->GetXaxis()->GetNbins(),hcdb_P_pr_neg[i]->GetXaxis()->GetXmin(),hcdb_P_pr_neg[i]->GetXaxis()->GetXmax());
  hcdb_Z_neg[i] = new TH1F(Form("hIntDistortionZ_negz_%d_cdb",runs[i]),Form("dz;#phi (rad);dz (cm)"),hcdb_Z_pr_neg[i]->GetXaxis()->GetNbins(),hcdb_Z_pr_neg[i]->GetXaxis()->GetXmin(),hcdb_Z_pr_neg[i]->GetXaxis()->GetXmax());
  plot1D(hcdb_N_pr_pos[i],hcdb_N_pos[i],r0);
  plot1D(hcdb_R_pr_pos[i],hcdb_R_pos[i],r0);
  plot1D(hcdb_P_pr_pos[i],hcdb_P_pos[i],r0);
  plot1D(hcdb_Z_pr_pos[i],hcdb_Z_pos[i],r0);
  plot1D(hcdb_N_pr_neg[i],hcdb_N_neg[i],r0);
  plot1D(hcdb_R_pr_neg[i],hcdb_R_neg[i],r0);
  plot1D(hcdb_P_pr_neg[i],hcdb_P_neg[i],r0);
  plot1D(hcdb_Z_pr_neg[i],hcdb_Z_neg[i],r0);
...
for (int i = 0; i < nrun; i++)
{
  ...
  hmy_N_pos[i] = new TH1F(Form("hentries_posz_%d_my",runs[i]),Form("N;#phi (rad);N"),hmy_N_pr_pos[i]->GetXaxis()->GetNbins(),hmy_N_pr_pos[i]->GetXaxis()->GetXmin(),hmy_N_pr_pos[i]->GetXaxis()->GetXmax());
  hmy_R_pos[i] = new TH1F(Form("hIntDistortionR_posz_%d_my",runs[i]),Form("dR;#phi (rad);dR (cm)"),hmy_R_pr_pos[i]->GetXaxis()->GetNbins(),hmy_R_pr_pos[i]->GetXaxis()->GetXmin(),hmy_R_pr_pos[i]->GetXaxis()->GetXmax());
  hmy_P_pos[i] = new TH1F(Form("hIntDistortionP_posz_%d_my",runs[i]),Form("Rdphi;#phi (rad);Rdphi (rad)"),hmy_P_pr_pos[i]->GetXaxis()->GetNbins(),hmy_P_pr_pos[i]->GetXaxis()->GetXmin(),hmy_P_pr_pos[i]->GetXaxis()->GetXmax());
  hmy_Z_pos[i] = new TH1F(Form("hIntDistortionZ_posz_%d_my",runs[i]),Form("dz;#phi (rad);dz (cm)"),hmy_Z_pr_pos[i]->GetXaxis()->GetNbins(),hmy_Z_pr_pos[i]->GetXaxis()->GetXmin(),hmy_Z_pr_pos[i]->GetXaxis()->GetXmax());
  hmy_N_neg[i] = new TH1F(Form("hentries_negz_%d_my",runs[i]),Form("N;#phi (rad);N"),hmy_N_pr_neg[i]->GetXaxis()->GetNbins(),hmy_N_pr_neg[i]->GetXaxis()->GetXmin(),hmy_N_pr_neg[i]->GetXaxis()->GetXmax());
  hmy_R_neg[i] = new TH1F(Form("hIntDistortionR_negz_%d_my",runs[i]),Form("dR;#phi (rad);dR (cm)"),hmy_R_pr_neg[i]->GetXaxis()->GetNbins(),hmy_R_pr_neg[i]->GetXaxis()->GetXmin(),hmy_R_pr_neg[i]->GetXaxis()->GetXmax());
  hmy_P_neg[i] = new TH1F(Form("hIntDistortionP_negz_%d_my",runs[i]),Form("Rdphi;#phi (rad);Rdphi (rad)"),hmy_P_pr_neg[i]->GetXaxis()->GetNbins(),hmy_P_pr_neg[i]->GetXaxis()->GetXmin(),hmy_P_pr_neg[i]->GetXaxis()->GetXmax());
  hmy_Z_neg[i] = new TH1F(Form("hIntDistortionZ_negz_%d_my",runs[i]),Form("dz;#phi (rad);dz (cm)"),hmy_Z_pr_neg[i]->GetXaxis()->GetNbins(),hmy_Z_pr_neg[i]->GetXaxis()->GetXmin(),hmy_Z_pr_neg[i]->GetXaxis()->GetXmax());
  plot1D(hmy_N_pr_pos[i],hmy_N_pos[i],r0);
  plot1D(hmy_R_pr_pos[i],hmy_R_pos[i],r0);
  plot1D(hmy_P_pr_pos[i],hmy_P_pos[i],r0);
  plot1D(hmy_Z_pr_pos[i],hmy_Z_pos[i],r0);
  plot1D(hmy_N_pr_neg[i],hmy_N_neg[i],r0);
  plot1D(hmy_R_pr_neg[i],hmy_R_neg[i],r0);
  plot1D(hmy_P_pr_neg[i],hmy_P_neg[i],r0);
  plot1D(hmy_Z_pr_neg[i],hmy_Z_neg[i],r0);
...
for (int i=0; i<nrun; i++)
{
  ...
  can->Update();
  can->SaveAs(Form("figure/resid_vsP_%d.pdf",runs[i]));

  TLegend *legend = new TLegend(0.1, 0.1, 0.9, 0.9);
  legend->SetHeader(Form("Run %d, radius = %.3f cm", runs[i], r0));
  ...
  legend->Draw();
  can_leg->SaveAs(Form("figure/resid_vsP_%d_legend.pdf",runs[i]));
