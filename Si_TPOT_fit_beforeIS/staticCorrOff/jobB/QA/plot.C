...
void plot()
{
  ...
  for (int k=0; k<nrun; k++)
  {
    ...
    intree->SetBranchAddress("crossing",&crossing);
    ...
    TH2* h2_drphi_r = new TH2F("h2_drphi_r","Rd#phi vs. R @ |Z|<20;R (cm);Rd#phi (cm)",16,20,78,50,-3,3);
    ...
    for (int i=0; i<nevent; i++)
    {
      ...
      if (std::abs(tanAlpha) > 0.6 || std::abs(drphi) > 3) continue;
