...
//    actsFit->setUseMicromegas(G4TRACKING::SC_USE_MICROMEGAS);
  ...
  */
  ...
  genfitFit->Verbosity(0);
  genfitFit->set_fit_silicon_mms(G4TRACKING::SC_CALIBMODE);
  genfitFit->set_use_micromegas(G4TRACKING::SC_USE_MICROMEGAS);
  genfitFit->disableAverageCorr();
  genfitFit->set_svtx_seed_map_name("PrunedSvtxTrackSeedContainer");
  ...
  std::string tpcresidstring = tpc_residoutfile.Data();
  ...
  tpcSpaceChargeReconstruction->set_outputfile(tpcresidstring);
  tpcSpaceChargeReconstruction->set_svtx_track_map_name("SvtxSiliconMMTrackMap");
  tpcSpaceChargeReconstruction->disableAverageCorr();

  tpcSpaceChargeReconstruction->set_min_pt( 0.5 );
  tpcSpaceChargeReconstruction->requireCrossing(false);
  tpcSpaceChargeReconstruction->requireCM(true);
  tpcSpaceChargeReconstruction->setPCAzcut(10);
  tpcSpaceChargeReconstruction->setEtacut(0.25);

  tpcSpaceChargeReconstruction->set_double_param("spacecharge_max_talpha", 0.6);
  tpcSpaceChargeReconstruction->set_double_param("spacecharge_max_tbeta", 1.5);
  tpcSpaceChargeReconstruction->set_double_param("spacecharge_max_drphi", 2);
  tpcSpaceChargeReconstruction->set_double_param("spacecharge_max_dz",5);

  tpcSpaceChargeReconstruction->setMinRPhiErr(0.005);
  tpcSpaceChargeReconstruction->setMinZErr(0.01);

  // reconstructed distortion grid size (layer)
  tpcSpaceChargeReconstruction->set_grid_dimensions(48);

  // reconstructed distortion grid size (phi, r, z)
  tpcSpaceChargeReconstruction->set_grid_dimensions(36, 16, 80);
