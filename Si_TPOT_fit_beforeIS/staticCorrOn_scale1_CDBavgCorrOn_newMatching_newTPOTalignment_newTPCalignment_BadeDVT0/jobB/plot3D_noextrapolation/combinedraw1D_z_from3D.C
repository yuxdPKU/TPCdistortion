...
for (int i = 0; i < nrun; i++)
{
  ...
  std::pair<double,double> yrange_R = SetCommonYRange({h_R_central_pos[i],h_R_west_pos[i],h_R_east_pos[i],h_R_central_neg[i],h_R_west_neg[i],h_R_east_neg[i],hfit_R_zr_west_pos[i]});
...
for (int i=0; i<nrun; i++)
{
  //h_Z_central_pos[i]->SetMinimum(-0.5);
  //h_Z_central_pos[i]->SetMaximum(2.5);
...
for (int i=0; i<nrun; i++)
{
  //h_Z_central_neg[i]->SetMinimum(-2.5);
  //h_Z_central_neg[i]->SetMaximum(0.5);
